    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(this, &MainWindow::results_ready, this, &MainWindow::results_ready_print);
...
void MainWindow::on_pb_start_clicked()
{
    ...
    auto findMax = [&](QVector<double> res){
                                                qDebug() << QString::number(res.size()) << "\n";
                                                maxs = FindMax(res);
                                                mins = FindMin(res);
                                                ...
                                                //проверяем чем заполнился QVector<double> x
                                                for (int i = 0; i < 1000; ++i) {
                                                    qDebug() << QString::number(x[i]);
                                                }
                                                //заполняем график точками
                                                for (int i = 0; i < x.size(); i++) {
                                                    series->append(x[i], res[i]);
                                                }
...
void MainWindow::results_ready_print(QChartView* chV) {
    chV->chart()->createDefaultAxes();
    chV->show( );
}
